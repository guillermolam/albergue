...
describe("redsys crypto", () => {
  ...
  it("round-trips: a correctly signed notification verifies", () => {
    ...
    const notification = verifyNotification(
      config,
      paramsBase64,
      signatureUrlSafe,
    );
  ...
  it("rejects a tampered signature", () => {
    ...
    const { paramsBase64 } = simulateNotification(
      request.orderNumber,
      "0000",
      "4500",
    );
  ...
  it("rejects a notification signed for a different order", () => {
    ...
    expect(
      verifyNotification(config, other.paramsBase64, other.signatureUrlSafe),
    ).not.toBeNull();
  ...
  it("embeds the booking reference on UrlOK", () => {
    ...
    const params = JSON.parse(
      Buffer.from(request.paramsBase64, "base64").toString("utf8"),
    );

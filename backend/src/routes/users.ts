...
users.get('/', async (c: Context) => {
  try {
    ...
    throw new HTTPException(500, { message: `Failed to get users: ${String(error)}` });
  }
});
...
users.get('/:id', async (c: Context) => {
  try {
    ...
    throw new HTTPException(500, { message: `Failed to get user: ${String(error)}` });
  }
});
...
users.get('/stats', async (c: Context) => {
  try {
    ...
    throw new HTTPException(500, { message: `Failed to get user statistics: ${String(error)}` });
  }
});
...
users.post('/', async (c: Context) => {
  try {
    ...
    throw new HTTPException(400, { message: `Failed to create user: ${String(error)}` });
  }
});

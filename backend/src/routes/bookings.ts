...
function parseIsoDate(value: string, field: string): Date {
  ...
  if (Number.isNaN(date.getTime())) {
    throw new HTTPException(400, {
      message: `Invalid ${field}: expected ISO 8601 datetime`,
    });
  }
  return date;
}
...
bookings.post("/", async (c: Context) => {
  try {
    ...
    if (!body.pilgrimId || !body.checkInDate || !body.checkOutDate) {
      throw new HTTPException(400, {
        message:
          "Missing required fields: pilgrimId, checkInDate, checkOutDate",
      });
    }
    ...
    const booking = await createBooking({
      ...
      referenceNumber:
        ...
        `ALB-${randomBytes(6).toString("hex").toUpperCase()}`,
    ...
    throw new HTTPException(400, {
      message: `Failed to create booking: ${String(error)}`,
    });
  }
});
  ...
  try {
    ...
    if (!quote)
      throw new HTTPException(404, {
        message: "Bed not found or invalid dates",
      });
    ...
    if (error instanceof HTTPException) throw error;
    throw new HTTPException(400, {
      message: `Failed to compute quote: ${String(error)}`,
    });
  }
});
...
bookings.patch("/:id/status", async (c: Context) => {
  try {
    ...
    if (error instanceof HTTPException) throw error;
    throw new HTTPException(400, {
      message: `Failed to update booking status: ${String(error)}`,
    });
  }
});

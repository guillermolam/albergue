...
payments.get("/", async (c: Context) => {
  try {
    ...
    throw new HTTPException(500, {
      message: `Failed to get payments: ${String(error)}`,
    });
  }
});
...
payments.get("/:id", async (c: Context) => {
  try {
    ...
    throw new HTTPException(500, {
      message: `Failed to get payment: ${String(error)}`,
    });
  }
});
...
payments.get("/booking/:bookingId", async (c: Context) => {
  try {
    ...
    if (error instanceof HTTPException) throw error;
    throw new HTTPException(500, {
      message: `Failed to get payments by booking: ${String(error)}`,
    });
  }
});
...
payments.get("/pending", async (c: Context) => {
  try {
    ...
    throw new HTTPException(500, {
      message: `Failed to get pending payments: ${String(error)}`,
    });
  }
});
...
payments.get("/overdue", async (c: Context) => {
  try {
    ...
    throw new HTTPException(500, {
      message: `Failed to get overdue payments: ${String(error)}`,
    });
  }
});
...
payments.get("/stats", async (c: Context) => {
  try {
    ...
    throw new HTTPException(500, {
      message: `Failed to get payment statistics: ${String(error)}`,
    });
  }
});
...
payments.post("/intent", async (c: Context) => {
  ...
  if (!body?.bookingReference) {
    throw new HTTPException(400, {
      message: "Missing required field: bookingReference",
    });
  }
  ...
  if (!config) {
    throw new HTTPException(501, {
      message:
        "Payments are not configured on this deployment (REDSYS_* env vars missing).",
    });
  }
  ...
  await createPayment({
    ...
    paymentDeadline:
      booking.paymentDeadline ?? new Date(Date.now() + 48 * 60 * 60 * 1000),
  });
...
payments.post("/redsys/notification", async (c: Context) => {
  ...
  if (isApproved(notification.responseCode)) {
    ...
    await markPaymentAsFailed(
      payment.id,
      `Redsys Ds_Response ${notification.responseCode}`,
    );

...
pricing.get("/", async (c: Context) => {
  try {
    ...
    throw new HTTPException(500, {
      message: `Failed to get pricing: ${String(error)}`,
    });
  }
});
...
pricing.get("/active", async (c: Context) => {
  try {
    ...
    throw new HTTPException(500, {
      message: `Failed to get active pricing: ${String(error)}`,
    });
  }
});
...
pricing.get("/room-type/:roomType", async (c: Context) => {
  try {
    ...
    throw new HTTPException(500, {
      message: `Failed to get pricing by room type: ${String(error)}`,
    });
  }
});
...
pricing.get("/stats", async (c: Context) => {
  try {
    ...
    throw new HTTPException(500, {
      message: `Failed to get pricing statistics: ${String(error)}`,
    });
  }
});

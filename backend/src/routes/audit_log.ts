...
auditLog.get('/', async (c: Context) => {
  try {
    ...
    throw new HTTPException(500, { message: `Failed to get audit logs: ${String(error)}` });
  }
});
...
auditLog.get('/table/:tableName', async (c: Context) => {
  try {
    ...
    throw new HTTPException(500, { message: `Failed to get audit logs by table: ${String(error)}` });
  }
});
...
auditLog.get('/action/:action', async (c: Context) => {
  try {
    ...
    throw new HTTPException(500, { message: `Failed to get audit logs by action: ${String(error)}` });
  }
});
...
auditLog.get('/stats', async (c: Context) => {
  try {
    ...
    throw new HTTPException(500, { message: `Failed to get audit log statistics: ${String(error)}` });
  }
});

/**
 * GET /audit-log/:id - Get audit log entry by ID
...
auditLog.get('/:id', async (c: Context) => {
  try {
    ...
    if (isNaN(id)) throw new HTTPException(400, { message: 'Invalid audit log ID' });
    ...
    throw new HTTPException(500, { message: `Failed to get audit log: ${String(error)}` });
  }
});

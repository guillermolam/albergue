...
export function BedManagement() {
  ...
  const getStatusBadge = (status: BedStatus) => {
    switch (status) {
      case "available":
        return (
          <Badge className="bg-green-100 text-[#00AB39] border-green-200">
            Available
          </Badge>
        );
      case "occupied":
        return (
          <Badge className="bg-red-100 text-[#ED1C24] border-red-200">
            Occupied
          </Badge>
        );
      case "reserved":
        return (
          <Badge className="bg-yellow-100 text-[#EAC102] border-yellow-200">
            Reserved
          </Badge>
        );
      case "maintenance":
  ...
  const maintenanceCount = allBeds.filter(
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {dorm1Beds.map((bed) => (
              <Card
                key={bed.id}
                className={`border-2 ${getBedColor(bed.status)}`}
              >

...
export function BedSelectionStep({ onNext, onBack }: BedSelectionStepProps) {
  ...
  const [dorm1Beds, setDorm1Beds] = useState<
  ...
  const [dorm2Beds, setDorm2Beds] = useState<
    ("available" | "selected" | "occupied")[]
  >(
    Array.from({ length: 12 }, (_, i) =>
  ...
  const selectedBedNumbers = [
    ...dorm1Beds
      .map((status, i) => (status === "selected" ? i + 1 : null))
      .filter(Boolean),
    ...dorm2Beds
      .map((status, i) => (status === "selected" ? i + 13 : null))
      .filter(Boolean),

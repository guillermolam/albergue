...
- `transition:persist`: Allows you to override Astro’s default replacing old elements for new ones and instead when navigating to another page.
...
The `data-astro-history` attribute can be set to the same three values as the [`history` option of the `navigate()` function](https://docs.astro.build/en/reference/modules/astro-transitions/#history-option):
...
Consider enabling Astro’s [Content Security Policy feature](https://docs.astro.build/en/reference/configuration-reference/#securitycsp) to help protect against cross-site scripting (XSS) risks if using user input with the `navigate()` API.

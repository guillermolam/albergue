...
function draw(svg: SVGSVGElement, cfg: RoughConfig) {
  ...
  if (cfg.texture !== "none") {
    ...
    const tex = rc.path(
      roundedPath(8, 8, 84, 24, Math.max(8, cfg.radius - 6)),
      {
        seed: cfg.seed + 1,
        stroke: "currentColor",
        strokeWidth: 1,
...
function initOne(root: HTMLElement) {
  ...
  const cfg: RoughConfig = {
    ...
    textureOpacity: numAttr(
      root,
      "data-rough-texture-opacity",
      DEFAULTS.textureOpacity,
    ),
...
/**
 * Initialize Rough.js elements within a scope.
 * @param scope - The parent node to search within (defaults to document)
 */
export function initRough(scope: ParentNode = document) {
  const nodes = Array.from(
    scope.querySelectorAll<HTMLElement>("[data-rough-frame]"),
  );
...
/**
 * Setup Rough.js initialization for initial load and Swup integration.
 * Handles SSR safety by checking if window is defined.
 */
export function setupRough() {
  // Initialize on initial load
  if (typeof window !== "undefined") {
    initRough(document);

    // Setup Swup hook if Swup is available
    declare global {
      interface Window {
        swup?: {
          hooks: {
    ...
    // Check if Swup is already available
    if (window.swup && !window.__albergueSwupHooks) {
      ...
      // Already initialized, just add the hook
      window.swup.hooks.on("content:replace", () => {
        initRough(document);
        window.dispatchEvent(new CustomEvent("swup:content:replaced"));
      });
...
// Initialize on initial load if in browser
if (typeof window !== "undefined") {
  setupRough();
}

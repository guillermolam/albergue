...
export function EmailProviderModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="border-2 border-[#1A1A1A] bg-[#FFF8E7] shadow-[8px_8px_0_0_#1A1A1A]">
        <DialogHeader>
          <DialogTitle className="text-xl font-black font-sketch">{t.title}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-2">
          {providers.map((provider) => (
            <a
              ...
              className={`${NEO_INTERACTIVE} ${provider.bg} flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-black text-[#1A1A1A]`}
            >
              {provider.icon}
              {provider.label}
            </a>
